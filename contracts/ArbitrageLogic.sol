...
contract ArbitrageLogic {
    ...
    function getLiveV4V3RouteParams(
        ...
        uint16 directionalProtocolFee =
            v4Token0 == startToken ? protocolFee.getZeroForOneFee() : protocolFee.getOneForZeroFee();
        uint24 v4Fee = directionalProtocolFee == 0 ? lpFee : directionalProtocolFee.calculateSwapFee(lpFee);

        return _getV4V3RouteParams(
            PoolStatesForIteration({
                sqrtPrice: sqrtPriceX96, tick: tick, liquidity: manager.getLiquidity(poolId), token0: v4Token0
            }),
            v4Fee,
            startToken,
            intermediateToken,
    ...
    function _getV4V3RouteParams(
        ...
        if (
            (externalPool.poolType != ArbUtils.PoolType.V3 && externalPool.poolType != ArbUtils.PoolType.PANCAKESWAP_V3)
        ...
        if (externalPool.poolType == ArbUtils.PoolType.V3) {
            ...
            try IPancakeV3Pool(externalPool.poolAddress).slot0() returns (
                uint160 sqrtPriceX96, int24 tick, uint16, uint16, uint16, uint32, bool
            ) {
                externalSqrtPriceX96 = sqrtPriceX96;
                externalTick = tick;
            } catch {
        ...
        uint256 v4RawPrice = getRawPriceScaled(
            v4State.sqrtPrice, startIsToken0, externalPool.token0Decimals, externalPool.token1Decimals
        );
        uint256 externalRawPrice = getRawPriceScaled(
            externalSqrtPriceX96, startIsToken0, externalPool.token0Decimals, externalPool.token1Decimals
        );
        if (getEffectiveSellPrice(v4RawPrice, v4Fee) <= getEffectiveBuyPrice(externalRawPrice, externalPool.fee)) {
            return route;
        }

        uint256 initialSpread = config.initialAbsSpread > 0 ? uint24(config.initialAbsSpread) : uint24(route.spread);
        if (config.maxImpactBps == 0) return route;
        uint256 move =
            (uint24(route.spread)
                    * (uint256(config.chunkSpreadConsumptionBps) + (2000 * uint24(route.spread)) / initialSpread))
                / (2 * BPS_DIVISOR);
        if (move == 0) move = 1;
        ...
        int256 targetExternalTick = int256(externalTick) + (zeroForOneExternal ? -int256(move) : int256(move));
        ...
        (uint256 startIn, uint256 intermediateOut) =
            ArbMath._deltaAmounts(zeroForOneV4, v4State.sqrtPrice, route.sqrtPriceLimitX96, v4State.liquidity);
        (uint256 externalCapacity,) = ArbMath._deltaAmounts(
            zeroForOneExternal, externalSqrtPriceX96, route.externalSqrtPriceLimitX96, externalLiquidity
        );
        if (startIn == 0 || intermediateOut == 0 || externalCapacity == 0) return route;

        route.principal =
            intermediateOut > externalCapacity ? FullMath.mulDiv(startIn, externalCapacity, intermediateOut) : startIn;
    ...
    function getV3SwapParameters(
        ...
        unchecked {
            ...
            uint256 tmpMove = (uint256(curSpreadAbs) * moveBpsAdaptive) /
                (2 * BPS_DIVISOR); // Preserve the original half-spread movement.
