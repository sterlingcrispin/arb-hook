...
contract ArbHookParityTest is Test {
    ...
    bytes32 private constant ARB_EVENT_TOPIC =
        keccak256("ArbitrageAttempted(address,address,address,address,uint256,int256,uint256)");
    ...
    function testOwnerOnlyAccessParity() public {
        vm.skip(bytes(baseRpcUrl).length == 0, "BASE_RPC_URL not configured");
    ...
    function testAttemptAllOnForkMatchesArbLightweightFlow() public {
        ...
        for (uint256 round = 0; round < MAX_ROUNDS; ++round) {
            ...
            if (!success) {
                ...
                emit log("no ArbitrageAttempted event emitted; inspect hook console output");
            }
        ...
        if (ENFORCE_PARITY) {
            assertEq(tradeCount, MAX_ROUNDS, "each round should record a trade while parity enforced");
        } else {
    ...
    function _logSummary(RoundResult[] memory actual, RoundExpectation[] memory expected, uint256 profitableRounds)
        private
    {
        emit log("");
        ...
        emit log(string.concat("Total USDC profit: ", vm.toString(profitWhole), ".", _padZeros(profitFrac, 6), " USDC"));
        ...
        for (uint256 i = 0; i < MAX_ROUNDS; ++i) {
            ...
            string memory profitStr = string.concat(vm.toString(pWhole), ".", _padZeros(pFrac, 6));

            bool matches = actual[i].cumulativeProfit == expected[i].profit && actual[i].buyPool == expected[i].buyPool
    ...
    function _deploySystem() private returns (DeployContext memory ctx) {
        ...
        ctx.hook.setChunkSpreadConsumptionBps(CHUNK_SPREAD_CONSUMPTION_BPS);
        ctx.hook.setMaxImpactBps(MAX_IMPACT_BPS);
        ctx.hook.setMinProfitToEmit(MIN_PROFIT_TO_EMIT);
    }

    function _registerParityPools(DeployContext memory ctx, PoolSpec[] memory specs) private {
        for (uint256 i = 0; i < specs.length; ++i) {
            ...
            try ctx.hook.addPools(specs[i].base, poolAddresses, fees, types) {} catch {} // Ignore duplicates just like JS harness
        }
    }

    function _approveParityPools(DeployContext memory ctx, PoolSpec[] memory specs) private {
    ...
    function _collectPoolAddresses(PoolSpec[] memory specs) private pure returns (address[] memory pools) {
    ...
    function _parityPools() private pure returns (PoolSpec[] memory specs) {
        ...
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x1C450D7d1FD98A0b04E30deCFc83497b33A4F608, fee: 200, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xd0b53D9277642d899DF5C87A3966A349A798F224, fee: 500, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC,
        ...
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x6c561B446416E1A00E8E93E221854d6eA4171372, fee: 3000, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xb4CB800910B228ED3d0834cF79D697127BBB00e5, fee: 100, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x56C8989222ed293E3c4a22628d8BCA633cE1eb99, fee: 400, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC,
            pool: 0xE9d76696f8A35e2E2520e3125875C3af23f1E69c,
            fee: 2500,
            poolType: ArbUtils.PoolType.PANCAKESWAP_V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x0b1C2DCbBfA744ebD3fC17fF1A96A1E1Eb4B2d69, fee: 10000, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xfBB6Eed8e7aa03B138556eeDaF5D271A5E1e43ef, fee: 500, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC,
        ...
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x3e7586D52A9D07F8611B8ecf6CCc8a689c34a659, fee: 10000, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xE9e25E35aa99A2A60155010802b81A25C45bA185, fee: 100, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xeC558e484cC9f2210714E345298fdc53B253c27D, fee: 3000, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0xEdc625B74537eE3a10874f53D170E9c17A906B9c, fee: 3000, poolType: ArbUtils.PoolType.V3
        });
        specs[idx++] = PoolSpec({
            base: USDC, pool: 0x36B4869995672DF7E3aFc36BE795Dbb998Bc639d, fee: 10000, poolType: ArbUtils.PoolType.V3
        });
    }

    function _expectedRounds() private pure returns (RoundExpectation[] memory rounds) {
    ...
    function _decodeAttempt(Vm.Log[] memory logs) private pure returns (RoundResult memory result) {
        for (uint256 i = 0; i < logs.length; ++i) {
            if (logs[i].topics.length == 4 && logs[i].topics[0] == ARB_EVENT_TOPIC) {
                ...
                (result.sellPool, result.totalAmountSwapped, result.cumulativeProfit, result.iterations) =
    ...
    function _logRound(uint256 round, RoundResult memory actual, RoundExpectation memory expected) private {
    ...
    function _assertRoundMatches(RoundResult memory actual, RoundExpectation memory expected, uint256 round)
        private
        pure
    {
